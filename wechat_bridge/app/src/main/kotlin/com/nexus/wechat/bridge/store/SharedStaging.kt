...
/**
 * Stage files so the WeChat process can read them.
 * Tries Magisk `su 0`, then `/storage/emulated/0/nexus_wechat`.
 */
object SharedStaging {
    private const val TAG = "WeChatStaging"
    private const val TMP_ROOT = "/data/local/tmp/nexus_wechat"
    ...
    fun ensureDirs() {
        ...
        for (root in listOf(File("/sdcard"), File("/storage/emulated/0"))) {
            try {
                File(root, "nexus_wechat/in").mkdirs()
                File(root, "nexus_wechat/out").mkdirs()
            } catch (_: Throwable) {
            }
        }
    }
    ...
    private fun stageOnPublic(src: File, sub: String, leaf: String): File? {
        ...
        for (root in roots.distinctBy { it.absolutePath }) {
            try {
                val dir = File(root, "nexus_wechat/$sub")
