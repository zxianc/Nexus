# Build sherpa-onnx-offline-tts for Android API 28 (needs prior ORT + clone from build_sherpa_android.ps1)
...
& $Ninja -j 8 sherpa-onnx-offline-tts
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

New-Item -ItemType Directory -Force -Path "$Build\install\bin", "$Build\install\lib" | Out-Null
Copy-Item -Force "$Build\bin\sherpa-onnx-offline-tts" "$Build\install\bin\"
