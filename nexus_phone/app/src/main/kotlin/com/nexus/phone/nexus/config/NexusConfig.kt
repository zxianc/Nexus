...
data class NexusConfig(
    ...
    companion object {
        ...
        fun fromJson(json: String): NexusConfig {
            val parsed = gson.fromJson(json, NexusConfig::class.java) ?: return default()
            val hasTakeoverKey =
                try {
            ...
            // Gson may leave Kotlin non-null String fields as null when absent from JSON.
            return parsed.copy(
                sims = sims,
                ...
                llm =
                    (parsed.llm ?: LlmConfig()).let { llm ->
                        llm.copy(
                            model = (llm.model ?: "").ifBlank { LlmConfig().model },
                            apiKey = llm.apiKey ?: "",
                            baseUrl = (llm.baseUrl ?: "").ifBlank { LlmConfig().baseUrl },
                            systemPrompt =
                                (llm.systemPrompt ?: "").ifBlank { DEFAULT_SYSTEM_PROMPT },
                        )
                    },
                notify = parsed.notify ?: NotifyConfig(),
            )
        }
    }
}
