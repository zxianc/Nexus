...
- [x] 锁定版本小号登录后，`GET /v1/health` 为 `hook=connected` 且 `logged_in=true`  
- [x] 局域网其它设备可发送文本到好友 / 群，微信侧可见  
- [x] 群发可带 `ats`，对端收到真正 @（含 `notify@all`）  
- [ ] 可发送图片、文件（图片已通；非图片文件未实现）  
- [ ] 可接收文本 / 图片 / 文件，经 `/v1/events` 与 `/v1/media/{id}` 取回（文本/图片已通）  
